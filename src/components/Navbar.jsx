...
const Navbar = () => {
  ...
  useEffect(() => {
    if (accessToken) {
      axios
        .get(import.meta.env.VITE_SERVER_DOMAIN + "/notifications/new-notification", {
          headers: {
      ...
      <nav className="navbar z-50">
        ...
        >
          <input
            type="text"
            placeholder="Search"
            className="w-full md:w-auto bg-grey p-4 pl-6 pr-[12%] rounded-full placeholder:text-dark-grey md:pl-12 md:pr-14"
              ...
              <Link to="/dashboard/notifications">
                <button className="w-12 h-12 rounded-full relative bg-grey hover:bg-black/10">
                  ...
                  {new_notification_available ? (
                    <span className=" absolute bg-red rounded-full w-3 h-3 z-10 top-2 right-2"></span>
                  ) : (
                    ""
                  )}
                </button>
